import { HOOKS, MODULE_ID, MODULE_TITLE, PROVIDERS, SOCKET_MESSAGES, TEMPLATE_PATHS, TRIGGER_EVENTS, moduleError, moduleLog } from "./constants.js";
...
import { TriggerService } from "./execution/trigger-service.js";
import { getAiAuthoringGuide, getEncounterPackageSchema } from "./state/json-schema.js";
...
function createServices() {
  ...
  const trigger = new TriggerService({ store, controller, log });
  return { adapters, store, validation, log, authority, rollback, importExport, controller, trigger };
}
...
function summarizeSequence(sequence) {
  ...
  return {
    ...
    actionCount: beats.reduce((total, beat) => total + (Array.isArray(beat.actions) ? beat.actions.length : 0), 0),
    triggerCount: beats.reduce((total, beat) => total + (Array.isArray(beat.triggers) ? beat.triggers.length : 0), 0),
    archived: sequence?.archived === true,
...
function makePublicApi() {
  return {
    ...
    getJsonSchema() {
      return getEncounterPackageSchema();
    },
    async getEncounterAuthoringContext() {
      assertGm();
      const actionTypes = await ensureServices().validation.getActionPickerEntries();
      return getAiAuthoringGuide(actionTypes);
    },
    async readActionTypeCatalog() {
      assertGm();
      return ensureServices().validation.getActionPickerEntries();
    },
    async exportEncounterJson(options = {}) {
      assertGm();
      return ensureServices().importExport.exportPayload(options ?? {});
    },
    validateEncounterJson(input) {
      assertGm();
      const payload = ensureServices().importExport.parsePayload(input);
      return {
        ok: true,
        ...
        sequences: payload.sequences.map(summarizeSequence),
        referenceMetadata: payload.referenceMetadata ?? []
      };
    },
    async importEncounterJson(input, options = {}) {
      assertGm();
      const safeOptions = options ?? {};
      const result = await ensureServices().importExport.importPayload(input, {
        scene: safeOptions.scene ?? undefined,
        mode: safeOptions.mode ?? "duplicate"
      });
      return {
        imported: result.imported.map(summarizeSequence),
        referenceMetadata: result.referenceMetadata ?? []
      };
    },
    async upsertSequence(sequence, options = {}) {
      assertGm();
      const safeOptions = options ?? {};
      const saved = await ensureServices().importExport.upsertSequence(sequence, {
        scene: safeOptions.scene ?? undefined,
        replace: safeOptions.replace !== false
      });
      return summarizeSequence(saved);
    },
    ...
    evaluateTriggers(scene = null, context = {}) {
      assertGm();
      return ensureServices().trigger.evaluate("api", scene ?? undefined, context ?? {});
    },
    simulateCombatTrigger(eventOrContext = TRIGGER_EVENTS.COMBAT_STARTED, options = {}) {
      assertGm();
      const event = typeof eventOrContext === "string"
        ? eventOrContext
        : eventOrContext?.event ?? TRIGGER_EVENTS.COMBAT_STARTED;
      const config = typeof eventOrContext === "string" ? options ?? {} : eventOrContext ?? {};
      return ensureServices().trigger.simulateCombatEvent(event, config, config.scene ?? undefined);
    },
    resetTriggerState(sequenceId, scene = null) {
      assertGm();
      return ensureServices().trigger.resetStateForSequence(sequenceId, scene ?? undefined);
    },
