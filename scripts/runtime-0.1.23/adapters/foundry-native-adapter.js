...
export class FoundryNativeAdapter {
  ...
  async validateConfig(action) {
    ...
    switch (action.type) {
      ...
      case "native.showJournalHandout": {
        ...
        if (config.userIds !== undefined && !Array.isArray(config.userIds)) throw new Error("Journal target userIds must be an array.");
        const missingUsers = (config.userIds ?? []).filter((userId) => !game.users?.has?.(userId));
        if (missingUsers.length) throw new Error(`Journal target users could not be resolved: ${missingUsers.join(", ")}`);
        return;
      }
  ...
  async showJournalHandout(action) {
    ...
    const userIds = Array.isArray(config.userIds) ? config.userIds.filter((userId) => game.users?.has?.(userId)) : [];
    const result = await this.showJournalDocument(
      page ?? doc,
      safeBoolean(config.showToPlayers, true),
      safeBoolean(config.createChatLinkFallback, true),
      userIds
    );
    return createResult(result.shown ? RESULT_STATUS.SUCCESS : RESULT_STATUS.WARNING, result.message, result.details);
  }

  async showJournalDocument(doc, showToPlayers = true, createChatLinkFallback = true, userIds = []) {
    if (showToPlayers) {
      const journalCollection = globalThis.Journal ?? globalThis.foundry?.documents?.collections?.Journal;
      if (typeof journalCollection?.show === "function") {
        await journalCollection.show(doc, { force: true, users: userIds });
        const audience = userIds.length ? `${userIds.length} selected player(s)` : "all players";
        return { shown: true, message: `Showed Journal handout: ${doc.name}`, details: { journalUuid: doc.uuid, mode: "players", userIds, audience } };
      }
    }
    const mode = showToPlayers ? "players" : "text";
    if (typeof doc?.show === "function") {
      await doc.show(showToPlayers);
      return { shown: true, message: `Showed Journal handout: ${doc.name}`, details: { journalUuid: doc.uuid, mode, userIds } };
    }
    if (doc?.parent && typeof doc.parent.show === "function") {
      await doc.parent.show(showToPlayers);
      return { shown: true, message: `Showed Journal handout: ${doc.parent.name}`, details: { journalUuid: doc.parent.uuid, pageUuid: doc.uuid, mode, userIds } };
    }
    if (createChatLinkFallback) {
      await ChatMessage.create({
        content: `<p><strong>Handout:</strong> @UUID[${doc.uuid}]{${escapeHtml(doc.name)}}</p>`,
        speaker: ChatMessage.getSpeaker?.({ user: game.user }) ?? {},
        ...(userIds.length ? { whisper: userIds } : {})
      });
      return { shown: false, message: "Journal show API was unavailable; posted a chat link instead.", details: { journalUuid: doc.uuid, fallback: "chatLink", userIds } };
    }
    return { shown: false, message: "Journal show API was unavailable.", details: { journalUuid: doc.uuid, userIds } };
  }
