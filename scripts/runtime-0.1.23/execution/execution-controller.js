...
export class ExecutionController {
  ...
  async dryRunBeat(sequenceId, beatId, scene = this.store.getActiveScene(), options = {}) {
    ...
    return this.validation.validateBeat(sequence, beat, { dryRun: true, triggerContext: options.triggerContext ?? null });
  }
  ...
  async runAction(sequenceId, beatId, actionId, options = {}) {
    ...
    try {
      const result = await this.executeOne(action, {
        ...
        executionId,
        singleAction: true,
        confirmedDangerous: options.confirmedDangerous,
  ...
  async runBeat(sequenceId, beatId, options = {}) {
    ...
    try {
      const validation = await this.validation.validateBeat(sequence, beat, { executionId, triggerContext: options.triggerContext ?? null });
      const blocking = [...validation.results, ...(validation.triggerResults ?? [])]
        .filter((entry) => [RESULT_STATUS.FAILURE, RESULT_STATUS.UNSUPPORTED].includes(entry.result.status));
      ...
      for (const group of groups) {
        ...
        if (group.mode === EXECUTION_MODES.PARALLEL) {
          const settled = await Promise.allSettled(group.actions.map((action) => this.executeOne(action, {
            scene,
            sequence,
            beat,
            executionId,
          ...
          for (const action of group.actions) {
            if (runState.cancelled) break;
            results.push(await this.executeOne(action, {
              scene,
              sequence,
              beat,
              executionId,
