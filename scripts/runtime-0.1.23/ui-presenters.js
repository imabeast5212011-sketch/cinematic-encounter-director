...
const TRIGGER_EVENT_LABELS = Object.freeze({
  [TRIGGER_EVENTS.COMBAT_STARTED]: "Combat started",
  [TRIGGER_EVENTS.COMBAT_ROUND_STARTED]: "Combat round started",
  [TRIGGER_EVENTS.COMBAT_TURN_STARTED]: "Combat turn started",
  [TRIGGER_EVENTS.INITIATIVE_REACHED]: "Initiative reached",
  [TRIGGER_EVENTS.COMBAT_ENDED]: "Combat ended",
...
export function summarizeTrigger(trigger = {}, targetBeat = null, targetSequence = null) {
  ...
  switch (trigger.event) {
    case TRIGGER_EVENTS.COMBAT_STARTED:
      return `When combat starts, ${action.toLocaleLowerCase()} ${target}.`;
    case TRIGGER_EVENTS.COMBAT_ROUND_STARTED:
      return `When combat round ${trigger.round} starts, ${action.toLocaleLowerCase()} ${target}.`;
    case TRIGGER_EVENTS.COMBAT_TURN_STARTED:
      return `When combat turn ${trigger.turn} starts, ${action.toLocaleLowerCase()} ${target}.`;
    case TRIGGER_EVENTS.INITIATIVE_REACHED:
      return `When initiative is ${COMPARISON_LABELS[trigger.comparison]?.toLocaleLowerCase() ?? "at or below"} ${trigger.initiative}, ${action.toLocaleLowerCase()} ${target}.`;
    case TRIGGER_EVENTS.COMBAT_ENDED:
      return `When combat ends, ${action.toLocaleLowerCase()} ${target}.`;
...
export function triggerEditorModel(trigger = {}, index = 0, beats = [], sequences = []) {
  ...
  const usesCount = [
    TRIGGER_EVENTS.ENEMY_DEFEATED_COUNT,
    TRIGGER_EVENTS.COMBATANT_DEFEATED_COUNT,
    TRIGGER_EVENTS.TOKEN_DEFEATED,
    TRIGGER_EVENTS.ALLY_DEFEATED
  ].includes(trigger.event);
  const usesRound = [TRIGGER_EVENTS.COMBAT_ROUND_STARTED, TRIGGER_EVENTS.COMBAT_ROUND_AT_LEAST].includes(trigger.event);
  const usesTurn = trigger.event === TRIGGER_EVENTS.COMBAT_TURN_STARTED;
  const usesInitiative = trigger.event === TRIGGER_EVENTS.INITIATIVE_REACHED;
  const usesHp = trigger.event === TRIGGER_EVENTS.TOKEN_HP_AT_OR_BELOW;
  const usesWatchRefs = usesCount || usesHp;
  const usesComparison = usesCount || trigger.event === TRIGGER_EVENTS.COMBAT_ROUND_AT_LEAST || usesHp || usesInitiative;
  return {
    ...trigger,
    index: index + 1,
    ...
    comparisonOptions: Object.entries(COMPARISON_LABELS).map(([value, label]) => displayChoice(value, trigger.comparison, label)),
