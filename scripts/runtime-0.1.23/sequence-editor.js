...
export class SequenceEditor extends HandlebarsApplicationMixin(ApplicationV2) {
  ...
  async handleAction(dataset) {
    try {
      switch (dataset.action) {
        ...
        case "add-combat-start-trigger":
          return this.addCombatStartTrigger();
        case "add-round-trigger":
          return this.addRoundTrigger();
        case "add-combat-end-trigger":
          return this.addCombatEndTrigger();
        ...
        case "simulate-combat-trigger":
          return this.simulateCombatTrigger(dataset);
        case "add-beat":
          return this.addBeat();
        case "select-beat":
  ...
  collectTriggerFormData(data, existingTriggers) {
    return existingTriggers.map((trigger, index) => {
      ...
      return normalizeTrigger({
        ...
        turn: data[`${prefix}-turn`] ?? trigger.turn,
        initiative: data[`${prefix}-initiative`] ?? trigger.initiative,
        threshold: data[`${prefix}-threshold`] ?? trigger.threshold,
        thresholdType: data[`${prefix}-thresholdType`] ?? trigger.thresholdType,
        comparison: data[`${prefix}-comparison`] ?? (eventChanged ? undefined : trigger.comparison),
  ...
  async simulateCombatTrigger(dataset = {}) {
    const scene = activeScene();
    if (!scene) {
      notify(createResult(RESULT_STATUS.WARNING, "No active Scene is available for trigger simulation."));
      return;
    }
    const results = await this.services.trigger.simulateCombatEvent(dataset.event, {
      round: dataset.round,
      turnNumber: dataset.turnNumber,
      initiative: dataset.initiative
    }, scene);
    const firedCount = Array.isArray(results) ? results.length : 0;
    notify(createResult(
      firedCount ? RESULT_STATUS.DRY_RUN : RESULT_STATUS.WARNING,
      firedCount ? `Combat trigger dry run matched ${firedCount} Trigger(s).` : "Combat trigger dry run matched no Triggers."
    ));
    this.render({ force: true });
  }

  async addCombatStartTrigger() {
    await this.appendTrigger({
      name: "Combat starts",
      event: TRIGGER_EVENTS.COMBAT_STARTED,
  ...
  async addCombatEndTrigger() {
    await this.appendTrigger({
      name: "Combat ends",
      event: TRIGGER_EVENTS.COMBAT_ENDED,
      action: TRIGGER_ACTIONS.SELECT_BEAT,
      once: true,
      enabled: true
    });
